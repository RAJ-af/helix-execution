...
import com.helix.app.core.data.remote.sse.SseClient
...
class ChatRepositoryImpl @Inject constructor(
    ...
    override suspend fun sendMessage(conversationId: Int, content: String): Resource<List<MessageEntity>> {
        return try {
            ...
            val userMsg = MessageEntity(System.currentTimeMillis().toInt(), conversationId, content, "user", "")
            messageDao.insertMessages(listOf(userMsg))
    ...
    override fun streamChat(conversationId: Int): Flow<SseStreamEvent> {
        val url = "http://10.0.2.2:8000/api/v1/conversations/$conversationId/stream"
